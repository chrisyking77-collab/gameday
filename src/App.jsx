...
const CONFS=["ALL","W/ ODDS","TOURNEY","SEC","Big Ten","Big 12","ACC","Big East","WCC","MWC","AAC","Pac-12","A-10","A Sun","Big Sky","Big South","Big West","CAA","CUSA","Horizon","Ivy","MAAC","MAC","MEAC","Missouri Valley","NEC","OVC","Patriot","SoCon","Southland","Summit","Sun Belt","SWAC","WAC","WCC","America East","Atlantic Sun"];
...
function Scores({sport,title,yearRange,hasWeeks,weekCount,defYear,color,extra,showConf}){
  ...
  useEffect(()=>{setLd(true);setErr(null);setGames([]);(async()=>{
    const lim=sport==="cbb"?400:200;
    if(view==="today"||view==="upcoming"){const days=view==="today"?3:7;const all=[];const seen=new Set();
      for(let i=0;i<days;i++){const d=await ef(`${E[sport]}/scoreboard?dates=${ds(i)}&limit=${lim}${extra?"&"+extra:""}`);(d?.events||[]).forEach(ev=>{if(!seen.has(ev.id)){seen.add(ev.id);all.push(ev);}});}
      setGames(all);if(!all.length)setErr("No games today — try 'By Week' view");setLd(false);return;}
    if(view==="yesterday"){const d=await ef(`${E[sport]}/scoreboard?dates=${ds(-1)}&limit=${lim}${extra?"&"+extra:""}`);setGames(d?.events||[]);setLd(false);return;}
    // By week
    for(const u of[
      `${E[sport]}/scoreboard?dates=${yr}&seasontype=${st}&week=${wk}&limit=${lim}`,
      `${E[sport]}/scoreboard?season=${yr}&seasontype=${st}&week=${wk}&limit=${lim}`,
      `${E[sport]}/scoreboard?dates=${yr}&seasontype=2&limit=${lim}`
  ...
  const filt=conf==="ALL"?games:conf==="W/ ODDS"?games.filter(ev=>ev.competitions?.[0]?.odds?.[0]?.details):conf==="TOURNEY"?games.filter(ev=>{const notes=ev.competitions?.[0]?.notes;return notes?.some(n=>(n.headline||"").toLowerCase().match(/tournament|conf.*tourn|semifinal|quarterfinal|championship|first four/));}):games.filter(ev=>{const cs=ev.competitions?.[0]?.competitors||[];return cs.some(c=>{const dn=(c.team?.displayName||"")+" "+(c.team?.shortDisplayName||"")+" "+(c.team?.name||"");const cn=ev.competitions?.[0]?.notes?.[0]?.headline||"";return dn.toLowerCase().includes(conf.toLowerCase())||cn.toLowerCase().includes(conf.toLowerCase());});});
